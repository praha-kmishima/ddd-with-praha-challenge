...
- ✅ チームリポジトリインターフェース
  ...
  - 必要なメソッドの設計
- ✅ チームリポジトリの実装
  - PostgreSQL実装（トランザクション管理によるデータ整合性の確保）
  - モックリポジトリを使用したテスト実装
  - データベーススキーマの拡張（teamsテーブルとteam_membersテーブルの追加）
...
3. **集約間の参照方法**
   - 集約間の参照をIDで行う方針は実装済み（TaskエンティティがTeamMemberをIDで参照）
   - 整合性確保の仕組みが未実装
