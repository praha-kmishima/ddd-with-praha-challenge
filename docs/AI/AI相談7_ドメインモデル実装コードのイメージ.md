...
```
ドメインモデルの、各モデルの実装イメージをtypescriptで記載したマークダウンを作成してください。
詳細仕様については、docs/仕様.mdを、モデリングについてはドメインモデル図_4集約バージョンを参考にしてください

作成したドメインモデル図をもとに、実際のコードだとどのようになるかをイメージするためのドキュメントです。

プロジェクト構成については@README.md で、オニオンアーキテクチャによるバックエンドAPIサーバーをイメージしていますが、あくまでこれは参考で、まずはドメインモデルの実装と、リポジトリの層と、アプリケーション層の責務を設計してください
```
